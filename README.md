...
Открываем WSL (так как я на Windows), где установлен Docker, переходим в каталог с `docker-compose.yaml` и выполняем команду:
...
- Переместим файл `dag.py` в каталог `dags`.
...
Используем следующие учетные данные для входа:

- **Логин:** airflow  
- **Пароль:** airflow  

Мы попадаем на стартовую страницу Airflow, где будет доступен наш DAG из файла `dag.py`.
...
2. Нажимаем синий **+** для добавления нового подключения и заполните поля следующим образом:
...
- После настройки подключения к базе данных можно запустить DAG:
  ...
  1. На странице Airflow найдем наш DAG   
  2. Нажимаем на **Trigger DAG** (треугольник) в правом верхнем углу страницы.  

- DAG начнёт выполняться, и мы сможеем отслеживать прогресс и логи задач в интерфейсе:
